/**************************************************************/
/* a Mills #15 simple sieve, v.1.1  (copyleft) 2024 S.Batalov */
/**************************************************************/
...
int main(int argc,char **argv) {
  ...
  if(argc<2 ) { printf(" Use: %s from_T to_T > output_file\n"
		       "\t a Mills #15 simple sieve, v.1.1\n"
		       "\t (copyleft) 2024 S.Batalov\n",argv[0]); exit(0); }
  l = atoll(argv[1]);
  l2= atoll(argv[2]);
  ...
  if(l > 5ULL) { 
    FILE *fp=fopen("SI1511","r"); // save sieved values from 0-1T run here
